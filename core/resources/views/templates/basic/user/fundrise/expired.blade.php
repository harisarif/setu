    ...
    <div class="row justify-content-center mt-5">
        <div class="table-responsive--md  table-responsive">
            <table class="table table-bordered table--light">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td data-label="@lang('Status')">
                            @if($item->approval == 1 && Carbon\Carbon::now() < $item->dealine)
                             <i class="fa fa-check"></i> @lang('Approved')
                             @else
                             <i class="fa fa-times text-danger"></i> @lang('Expired')
                             @endif
                        </td>
                        <td data-label="@lang('Action')" class="text-center">

                        @if(Carbon\Carbon::now() > $item->dealine)
                            <a href="{{route('user.fundrise.edit',['slug'=>slug($item->title),'id'=>$item->id])}}">
                                <i title="Edit" class="fas fa-pen bg-primary text-white p-2 rounded"></i>
                            </a>
                        @else
                ...
                @empty
                <tr>
                    <td colspan="100%"><p class="text-center font--size"><i class="fas fa-laugh"></i> @lang('Campaign not Completed Yet') <i class="fas fa-laugh"></i></p></td>
                </tr>
