              ...
              <div class="table-responsive--md table-responsive">
                      ...
                      @forelse($comment as $key => $com)
                          <tr class="filt">
                              ...
                              <td data-label="@lang('Success Story')">
                              <a href="{{route('admin.success.details',['slug' => $com->blog->slug,'id'=>$com->blog->id])}}">{{shortDescription($com->blog->title,20)}}</a>  
                                </td>
                              <td data-label="@lang('Name')">{{$com->name}}</td>
                              <td data-label="@lang('Email')">{{$com->email}}</td>
                              <td data-label="@lang('Details')">{{Str::limit($com->messages,30)}}</td>
                              <td data-label="@lang('Review Date')">{{$com->created_at->diffForHumans()}}</td>
                              <td data-label="@lang('Status')">
                               
                                <span class="text--small badge font-weight-normal {{$com->status == 1 ? 'badge--success':($com->status == 2 ? "badge--danger":'badge--warning')}}">{{$com->status == 1 ? 'Published':($com->status == 2 ? 'Rejected': 'Pending')}}</span>
                              ...
                              <a href="javascript:void(0)" class="icon-btn btn--primary editbtn  ml-1" data-toggle="tooltip" title="" data-original-title="Edit" data-comment_id="{{$com->id}}" data-description="{{$com->messages}}" data-status = "{{$com->status}}" data-title = "{{$com->blog->title}}">
                                    <i class="la la-pen"></i>
                                </a>
