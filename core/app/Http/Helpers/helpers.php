...
function analytics()
{
    $reCaptcha = \App\Extension::where('act', 'google-analytics')->where('status', 1)->first();
    return $reCaptcha ? $reCaptcha->generateScript() : '';
}

function tawkto()
{
    $reCaptcha = \App\Extension::where('act', 'tawk-chat')->where('status', 1)->first();
    return $reCaptcha ? $reCaptcha->generateScript() : '';
}
...
function getCustomCaptcha($height = 46, $width = '300px', $bgcolor = '#003', $textcolor = '#abc')
{
    ...
    foreach ($char as $value) {
        $ret .= '<span style=" font-size:25px; padding-top:5px;color:#fff;    float:left;     -webkit-transform: rotate(' . rand(-60, 60) . 'deg);">' . $value . '</span>';
    }
    $ret .= '</div>';
...
{
    $credentials = GeneralSetting::first(['mail_config'])->mail_config;
    $mail = new PHPMailer();
    try {
        //Server settings
        // $mail->isSMTP();
        ...
        $mail->Port       = $credentials->port;        //Recipients
        $mail->setFrom($sender_email, $sender_name);
        $mail->addAddress($receiver_email, $receiver_name);
        $mail->addReplyTo($sender_email, $receiver_name);        // Content
        $mail->isHTML(true);
