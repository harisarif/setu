        ...
        public function remove(Request $request)
            {
                ...
                if ($type == 'element' || $type == 'content') {
                    ...
                    if (isset($sections->$key->$type->images) && is_array($sections->$key->$type->images)) {
                        $imgJson = $sections->$key->$type->images;

                        foreach ($imgJson as $kkk => $vvv) {
                            // Delete the files if they exist
                            $filePath = $path . '/' . $frontend->data_values->$kkk;
                            $thumbPath = $path . '/thumb_' . $frontend->data_values->$kkk;
                    ...
                    // Handle cases where $type is not 'element' or 'content'
                    $frontend->delete();
                    $notify[] = ['success', 'Content has been removed.'];
                    return back()->withNotify($notify);
                }
