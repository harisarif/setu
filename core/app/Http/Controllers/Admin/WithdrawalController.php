...
class WithdrawalController extends Controller
{
    ...
    public function dateSearch(Request $request,$scope){
        ...
        $end = @$date[1];
        if ($start) {
            $withdrawals = Withdrawal::where('status','!=',0)->where('created_at','>=',Carbon::parse($start))->with(['user', 'method']);
        }
        if($end){
            $withdrawals = Withdrawal::where('status','!=',0)->where('created_at','<=',Carbon::parse($end))->with(['user', 'method']);
        }
        if ($request->method) {
