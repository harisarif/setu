...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['name'] = Auth::user()->username ?? json_decode($general->anonymous)->username;
        $val['description'] = "Payment By Razorpay";
        $val['image'] = asset( 'assets/images/logoIcon/logo.png');
        $val['prefill.name'] = (Auth::user()->firstname?? json_decode($general->anonymous)->username) . ' ' . (Auth::user()->lastname?? json_decode($general->anonymous)->username) ;
        $val['prefill.email'] = Auth::user()->email ?? json_decode($general->anonymous)->email;
        $val['prefill.contact'] = Auth::user()->mobile ?? '123456789';
